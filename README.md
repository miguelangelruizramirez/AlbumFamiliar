# 📸 Álbum Familiar — Miguel & su Historia

Este repositorio existe para guardar recuerdos que no deben perderse:  
momentos con mi hijo, con Joss, con mi familia y conmigo mismo.

También forma parte de mi proceso personal de recuperación, reflexión y crecimiento.  
Aquí dejo imágenes, videos y documentos que marcaron etapas importantes de mi vida,  
y que quiero que mi hijo pueda ver cuando sea más grande.
---

## 🌟 Contenido del repositorio

### 📁 **Álbum (Fotos y Videos)**
- Todas las fotos están en `imagenes/`
- Todos los videos están en `videos/`
- El archivo `index.html` muestra automáticamente galeria

👉 **Abrir Álbum:**  

---

## 🧭 Propósito

**Conservar recuerdos familiares** de la manera más segura y accesible.
---

## ❤️ Dedicado a

- **Mi hijo:** Miguel Angel Ruiz Tlalpan  
- **Joss:** por los momentos que marcaron etapas reales  
- **Mi familia:** por sostenerme cuando no pude solo  
- **A mí mismo:** para recordar que siempre se puede volver a empezar

---

Miguel.
